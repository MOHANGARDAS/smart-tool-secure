// ═══════════════════════════════════════════════════════════
// SMART-TOOL/M — Secure Backend Server (Dynamic Users Version)
// ═══════════════════════════════════════════════════════════
'use strict';
...
// 🔒 Dynamic users list store karne ke liye (Memory Storage)
const dynamicUsers = new Map();
// Default admin-user ko pehle se add karke rakh rahe hain
dynamicUsers.set('mohan', '1121');
...
// ═══════════════════════════════════════════════════════════
// DYNAMIC LOGIN & CREATION ROUTES (REAL-TIME WORKING)
// ═══════════════════════════════════════════════════════════

// 1. User Login Route - Admin se banaye gaye saare users accept karega
app.post('/api/login', loginLimiter, upload.none(), async (req, res) => {
  try {
    ...
    // 🔍 Check karna ki kya ye user humari list (ya admin panel se banaya hua) me hai
    if (dynamicUsers.has(inputUser) && dynamicUsers.get(inputUser) === inputPass) {
...
// 3. Admin Panel - Fetch Users List
app.get('/api/admin/users', (req, res) => {
  const usersList = Array.from(dynamicUsers.keys()).map((user, idx) => ({
    id: `usr_${idx}`,
    username: user,
    online: activeSessions.has(user),
    blocked: false,
    lastLogin: new Date()
  }));
  res.json(usersList);
...
// 4. Admin Panel - Create / Add New User (Now fully functional!)
app.post('/api/admin/users', upload.none(), (req, res) => {
  ...
  // New user ko dynamically list me add karna
  dynamicUsers.set(newUser, newPass);
  
  res.json({ ok: true, message: `User ${newUser} created successfully` });
});
